...
@section('content')
    ...
    <h2 class="form-title"><i class="bi bi-calendar-event"></i> Modifier : {{ $trimestre->nom }}</h2>
        ...
        <form method="POST" action="{{ route('admin.trimestres.update', $trimestre->id) }}">
            ...
            <select name="annee_academique_id" id="annee_academique_id" required>
                @foreach ($annees as $annee)
                    <option value="{{ $annee->id }}" {{ old('annee_academique_id', $trimestre->annee_academique_id) == $annee->id ? 'selected' : '' }}>
                        {{ $annee->annee }}
                    </option>
                @endforeach
            </select>
